...
\begin{itemize}
  ...
  \item column dropping: The analysis began with loading the Adult dataset consisting of 15 columns initially. Several columns were considered non-quasi-identifying or as non-sensitive
...
most records being from the United States, with a smaller representation from other countries.
% ------------------------------------------------
...
\subsection{Results}
The performance of the proposed LSH-RC method was evaluated against a baseline k-means clustering approach across varying dataset sizes.
The key metrics assessed were execution time and information loss (ILoss) as functions of the number of records.
Overall our results indicate that LSH-RC scales much less efficiently than k-means while maintaining comparable ILoss levels.

\begin{figure}[h]
  \centering

  % --- Row 1 ---
  \subfloat[Execution Time A]{
    \includegraphics[width=0.45\linewidth]{images/time_a.png}
    \label{fig:a}
  }
  \hfill
  \subfloat[ILoss A]{
    \includegraphics[width=0.45\linewidth]{images/iloss_a.png}
    \label{fig:b}
  }
  ...
  % --- Row 2 ---
  \subfloat[Execution Time B]{
    \includegraphics[width=0.45\linewidth]{images/time_b.png}
    \label{fig:c}
  }
  \hfill
  \subfloat[ILoss B] {
    \includegraphics[width=0.45\linewidth]{images/iloss_b.png}
    \label{fig:d}
  }

  \caption{Execution time (a) \& (c), and ILoss (b) \& (d) w.r.t. \#(records).}
  \label{fig:fourgrid}
\end{figure}
...
\begin{itemize}
  ...
  \item Increasing leftovers records: Some iterations produced more leftover (\textless k)
